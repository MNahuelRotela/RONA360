...
import { MultimediaImagenes } from "./MultimediaImagenes";
import { MultimediaIframes } from "./MultimediaIfrems";
import { MultimediaAudio } from "./MultimediaAudio";
...
export const DetailPrensa = () => {
  ...
  useEffect(() => {
    ...
    const articleDetail = content?.prensa[language].articles.find(
      (article: Article) => slugify(article.title) === decodedTitle
    );
  ...
  return (
    <div className="mt-24 max-w-5xl m-auto px-6 2xl:px-0 xs:px-4">
      ...
      <div data-aos="fade-up">
        ...
        <h1 className="text-4xl font-medium mb-3 sm:text-2xl xs:text-2xl leading-tight">{article.title}</h1>
      ...
      {!article.videoUrl && article.gallery && (
        <MultimediaImagenes gallery={article.gallery}  />
      )}
      <div data-aos="fade-up">
      ...
      <div className="mt-28">
      {article.iframe && <MultimediaIframes iframes={article.iframe}  />}
      </div>
    </div>
  );
};